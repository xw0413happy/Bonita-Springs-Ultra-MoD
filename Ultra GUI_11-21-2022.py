...
# Update notes: change the range of the x-axis with datetimes and remove back (2022 Oct data is special)
# File Format: .py & .xlsx file
...
def run_py_btn (Ultra_data):
    ...
    ax = sns.countplot(x="date", hue="Status", data=trip_ID_df_range, palette = "Set2", linewidth=0.5)
    ...
    # Set up x-axis time range for weekdays
    ...
    # Set up x-axis time range for weekdays
    #start_time = hourly_resampled_weekdays_range['TIME_INTERVALS_str'][2]
    #end_time = hourly_resampled_weekdays_range['TIME_INTERVALS_str'][16]
...
root.title('Bonita Springs MoD Graph Version 3')
...
instro_cust = tk.Label(root, fg='white', text = " Image 7-9: Monthly Trip Integration \n Image 10-15: Customize the Date Range for Daily Trips", font=("shati"), bg="#6897bb") # bg="#a1caf1"
instro_cust_Font = font.Font(weight="bold", size=12)
instro_cust['font'] = instro_cust_Font 
instro_cust.grid(row=7, column=0, columnspan=2)
